...
async function callEdit(rule) {
    if (!currentDomain) {
        setStatus("No domain detected.", true);
        return;
    }

    setStatus(`Sending ${rule} rule...`);

    const url =
        `${BASE_URL}/edit/shadowrocket?` +
        new URLSearchParams({
...
function openConfig() {
    chrome.tabs.create({
        url: `${BASE_URL}/config/shadowrocket`,
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("btn-direct").addEventListener("click", () => callEdit("DIRECT"));

    document.getElementById("btn-proxy").addEventListener("click", () => callEdit("PROXY"));

    document.getElementById("btn-reject").addEventListener("click", () => callEdit("REJECT"));
